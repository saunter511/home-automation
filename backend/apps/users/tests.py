...
class UserTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(  # type: ignore
            self.email, self.password, birth_date=self.birth_date
        )
    ...
    def test_user_non_unique(self):
        with pytest.raises(IntegrityError):
            User.objects.create_user(self.email, self.password)  # type: ignore
    ...
    def test_user_fullname(self):
        self.assertEqual(
            ...
            self.email,
            "User full name didn't fallback to email",
        )
